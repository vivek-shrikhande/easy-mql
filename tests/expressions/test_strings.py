...
class TestStringsExpression:
    ...
    def test_concat(self):
        assert self.exp.parse('CONCAT("Hello", "easy", "mql!")') == {'$concat': ['Hello', 'easy', 'mql!']}
        with raises(ParseException):
            self.exp.parse('CONCAT("Hello")')

    def test_index_of_bytes(self):
        assert self.exp.parse('INDEX_OF_BYTES("foo.bar.fi", ".")') == {'$indexOfBytes': ['foo.bar.fi', '.']}
        assert self.exp.parse('INDEX_OF_BYTES("foo.bar.fi", ".", 5)') == {'$indexOfBytes': ['foo.bar.fi', '.', 5]}
        assert self.exp.parse('INDEX_OF_BYTES("vanilla", "ll", 0, 2)') == {'$indexOfBytes': ['vanilla', 'll', 0, 2]}
        with raises(ParseException):
    ...
    def test_index_of_cp(self):
        assert self.exp.parse('INDEX_OF_CP("foo.bar.fi", ".")') == {'$indexOfCP': ['foo.bar.fi', '.']}
        assert self.exp.parse('INDEX_OF_CP("foo.bar.fi", ".", 5)') == {'$indexOfCP': ['foo.bar.fi', '.', 5]}
        assert self.exp.parse('INDEX_OF_CP("vanilla", "ll", 0, 2)') == {'$indexOfCP': ['vanilla', 'll', 0, 2]}
        with raises(ParseException):
    ...
    def test_trim(self):
        assert self.exp.parse('TRIM(" ggoodbyeee ")') == {'$trim': {'input': ' ggoodbyeee '}}
        assert self.exp.parse('TRIM(" ggoodbyeee ", "ge")') == {'$trim': {'input': ' ggoodbyeee ', 'chars': 'ge'}}
        assert self.exp.parse('TRIM(" ggoodbyeee ", " gd")') == {'$trim': {'input': ' ggoodbyeee ', 'chars': ' gd'}}
        with raises(ParseException):
            self.exp.parse('TRIM(" ggoodbyeee ", " gd", "ee")')

    def test_ltrim(self):
        assert self.exp.parse('LTRIM(" ggoodbyeee ")') == {'$ltrim': {'input': ' ggoodbyeee '}}
        assert self.exp.parse('LTRIM(" ggoodbyeee ", "ge")') == {'$ltrim': {'input': ' ggoodbyeee ', 'chars': 'ge'}}
        assert self.exp.parse('LTRIM(" ggoodbyeee ", " gd")') == {'$ltrim': {'input': ' ggoodbyeee ', 'chars': ' gd'}}
        with raises(ParseException):
            self.exp.parse('LTRIM(" ggoodbyeee ", " gd", "ee")')

    def test_rtrim(self):
        assert self.exp.parse('RTRIM(" ggoodbyeee ")') == {'$rtrim': {'input': ' ggoodbyeee '}}
        assert self.exp.parse('RTRIM(" ggoodbyeee ", "ge")') == {'$rtrim': {'input': ' ggoodbyeee ', 'chars': 'ge'}}
        assert self.exp.parse('RTRIM(" ggoodbyeee ", " gd")') == {'$rtrim': {'input': ' ggoodbyeee ', 'chars': ' gd'}}
        with raises(ParseException):
            self.exp.parse('RTRIM(" ggoodbyeee ", " gd", "ee")')

    def test_regex_find(self):
        assert self.exp.parse('REGEX_FIND("Easymql", "ql")') == {'$regexFind': {'input': 'Easymql', 'regex': 'ql'}}
    ...
    def test_regex_find_all(self):
        assert self.exp.parse('REGEX_FIND_ALL("Easymql", "ql")') == {'$regexFindAll': {'input': 'Easymql', 'regex': 'ql'}}
    ...
    def test_regex_match(self):
        assert self.exp.parse('REGEX_MATCH("Easymql", "ql")') == {'$regexMatch': {'input': 'Easymql', 'regex': 'ql'}}
    ...
    def test_replace_all(self):
        assert self.exp.parse('REPLACE_ALL("Easymql= Easymql", "Easy", "easy")') == {
            '$replaceAll': {'input': 'Easymql= Easymql', 'find': 'Easy', 'replacement': 'easy'}
        }
        with raises(ParseException):
    ...
    def test_split(self):
        assert self.exp.parse('SPLIT("June-15-2013", "-")') == {'$split': ['June-15-2013', '-']}
        with raises(ParseException):
    ...
    def test_string_case_comparison(self):
        assert self.exp.parse('STRCASECMP("Easy", "easy")') == {'$strcasecmp': ['Easy', 'easy']}
        with raises(ParseException):
    ...
    def test_substr(self):
        assert self.exp.parse('SUBSTR("easymql", 0, 2)') == {'$substr': ['easymql', 0, 2]}
        with raises(ParseException):
    ...
    def test_substr_bytes(self):
        assert self.exp.parse('SUBSTR_BYTES("easymql", 0, 2)') == {'$substrBytes': ['easymql', 0, 2]}
        with raises(ParseException):
    ...
    def test_substr_cp(self):
        assert self.exp.parse('SUBSTR_CP("easymql", 0, 2)') == {'$substrCP': ['easymql', 0, 2]}
        with raises(ParseException):
